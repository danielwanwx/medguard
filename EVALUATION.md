...
Baseline artifacts are stored in `docs/evidence/baseline/`. The complete headless run finished at **2026-09-12 18:41 UTC**, using the frozen baseline production build on port 5175 and this checkout's real backend on 8910. [Browser evidence](docs/evidence/baseline/browser-evidence.json) and [captured API responses](docs/evidence/baseline/api-call-response.json) record three HTTP 200 responses, each with four actual Strands calls, and no harness failures. Saved screenshots cover desktop/mobile initial states, each product's loading and result state, mobile results, the doctor modal, and reload. These are observations of the defective baseline, not passing safety acceptance tests.

The pre-existing processes on ports 5173/8910 belonged to the previous `Agents-for-Humans` directory, discovered with `lsof`; they were replaced before evaluation. Localhost and external API calls require execution outside this session's network sandbox; a sandbox connection failure is not an application failure.
...
`cd ui && npm run build` passed on the baseline (30 modules, Vite 6.4.2). The headless run independently reproduced the Fish Oil false recall, missing dosage display, fabricated progress rows, and loss of all three cabinet items after reload. In this later run, St. John's Wort displayed a recall rather than CLEAR, while the model still falsely claimed “no known interactions” and dosage “within safe limits.” The differing recall output reinforces that the earlier manual outcome is not a deterministic fixture. Screenshots and captured JSON preserve the actual later response. Scores intentionally give no credit for undocumented vision, persistence, background monitoring, deployment or a missing public video.
